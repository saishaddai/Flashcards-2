...
class RoomStatsRepository(
    ...
    override fun getSkillMastery(): Flow<List<MasteryData>> {
        return studyDao.getAllDeckMastery().map { masteryLevels ->
            val allDecks = DeckType.entries.map { type ->
                val mastery = masteryLevels.find { it.deckId == type.id }
                if (mastery != null) {
                    ...
                    val friendlyName = type.friendlyName
    ...
    override fun getMasteredDecks(): Flow<String> {
        return studyDao.getAllDeckMastery().map { masteryLevels ->
            if (masteryLevels.isEmpty()) return@map "0%"
            val masteredCount = masteryLevels.count { it.progress >= 80.0 }
            val percentage = (masteredCount.toDouble() / masteryLevels.size * 100).toInt()
            "$percentage%"
        }
    }
